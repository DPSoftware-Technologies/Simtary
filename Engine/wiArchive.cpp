...
namespace wi
{
	...
	void Archive::SetReadModeAndResetPos(bool isReadMode)
	{
		...
		if (readMode)
		{
			...
			{
				wi::helper::messageBox("File is not supported!\nReason: The archive version (" + std::to_string(header.version) + ") is no longer supported! This is likely because trying to open a file that was created by a version of HMMWV4 that is too old.", "Error!");
				Close();
				return;
			}
			if (header.version > __archiveVersion)
			{
				wi::helper::messageBox("File is not supported!\nReason: The archive version (" + std::to_string(header.version) + ") is higher than the program's (" + std::to_string(__archiveVersion) + ")!\nThis is likely due to trying to open an Archive file that was not created by HMMWV4.", "Error!");
				Close();
				return;
			}
			if (GetVersion() >= 92)
