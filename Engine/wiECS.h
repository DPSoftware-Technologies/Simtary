...
namespace wi::ecs
{
	...
	struct EntitySerializer
	{
		...
		// Optional load progress callback, invoked once per component manager while reading (deserializing):
		//	total		: number of registered component managers found in the archive
		//	current		: 1-based index of the component manager currently being loaded
		//	component	: registered name of that component manager (e.g. "wi::scene::Scene::materials")
		std::function<void(uint32_t total, uint32_t current, const std::string& component)> progress = nullptr;
				...
				do
				{
					...
					if (has_next)
					{
						...
						if (it != entries.end())
						{
							...
							progress_total++; // only registered managers are actually deserialized in the second pass
						}
				...
				do
				{
					...
					if(has_next)
					{
						...
						if(it != entries.end())
						{
							...
							if (seri.progress)
							{
								// Report the component about to load (1-based), so "current" reflects what is loading now:
								seri.progress(progress_total, ++progress_current, name);
							}
