...
namespace wi::initializer
{
	...
	void InitializeComponentsAsync()
	{
		...
		// Engine audio now runs on the OpenAL subsystem (stAudio) instead of the
		// legacy XAudio2/FAudio backend. wi::audio::Initialize() is intentionally
		// NOT called, so wiAudio.cpp stays uninitialized: its Sound/Play API remains
		// linked but degrades to safe no-ops until stAudio implements it.
		wi::audio::InitializeOpenAL(); systems[INITIALIZED_SYSTEM_AUDIO].store(true);
