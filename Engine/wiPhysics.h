...
namespace wi::physics
{
	...
	// Returns true if a Jolt character has actually been created for this body (i.e. the
	// body was built while character physics was enabled). The CHARACTER_PHYSICS flag can
	// be set without a character existing yet (until the body is (re)built).
	bool HasCharacter(wi::scene::RigidBodyPhysicsComponent& physicscomponent);

	// Returns true if the body has a valid Jolt BodyID (i.e. the body/character was actually
	// created). False means creation failed (degenerate shape, body limit, ...).
	...
	// TEMP debug: character writeback tracing. Flags: bit0 reached feedback dispatch, bit1 passed
	// Dynamic gate, bit2 wrote transform. MotionType = motion type seen at the gate. Pos = written pos.
	int DbgCWBFlags();
	int DbgCWBMotionType();
	XMFLOAT3 DbgCWBPos();
