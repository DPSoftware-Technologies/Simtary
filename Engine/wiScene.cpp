...
namespace wi::scene
{
	...
	void Scene::Clear()
	{
		// Fire Destroy() on all native components before their entities/metadata disappear:
	...
	void Scene::MergeFastInternal(Scene& other)
	{
		...
		// Transfer native component runtime instances. Entities keep their IDs through a merge,
		// so we move the already-created instances over (rebinding them to this scene) instead of
		// letting this scene re-create + re-Start() them from the merged metadata. This is what
		// keeps Start() from firing twice when LoadModel() updates a temp scene before merging.
		for (auto& pair : other.nativeComponents.instances)
		{
			wi::vector<NativeComponentManager::Instance>& dst = nativeComponents.instances[pair.first];
			for (NativeComponentManager::Instance& inst : pair.second)
			{
				if (inst.component)
					inst.component->scene = this;
				dst.push_back(std::move(inst));
			}
		}
		other.nativeComponents.instances.clear();
	}
	...
	void Scene::Entity_Remove(Entity entity, bool recursive, bool keep_sorted)
	{
		...
		// Fire Destroy() on this entity's native components while its metadata still exists:
		nativeComponents.RemoveEntity(entity);
	...
	Entity LoadModel(const std::string& fileName, const XMMATRIX& transformMatrix, bool attached, LoadModelProgressCallback progress)
	{
		Entity rootEntity = INVALID_ENTITY;
		if (attached)
		{
		...
		LoadModel2(fileName, transformMatrix, rootEntity, progress);
		return rootEntity;
	}

	Entity LoadModel(Scene& scene, const std::string& fileName, const XMMATRIX& transformMatrix, bool attached, LoadModelProgressCallback progress)
	{
		Entity rootEntity = INVALID_ENTITY;
		if (attached)
		{
		...
		LoadModel2(scene, fileName, transformMatrix, rootEntity, progress);
		return rootEntity;
	}

	void LoadModel2(const std::string& fileName, const XMMATRIX& transformMatrix, Entity rootEntity, LoadModelProgressCallback progress)
	{
		Scene scene;
		// Reserve the last 5% of the bar for the Merge step into the global scene:
		LoadModelProgressCallback inner = progress;
		if (progress)
			inner = [progress](float fraction, const std::string& status) { progress(fraction * 0.95f, status); };
		LoadModel2(scene, fileName, transformMatrix, rootEntity, inner);
		if (progress) progress(0.97f, "Merging");
		GetScene().Merge(scene);
		if (progress) progress(1.0f, "Done");
	}

	void LoadModel2(Scene& scene, const std::string& fileName, const XMMATRIX& transformMatrix, Entity rootEntity, LoadModelProgressCallback progress)
	{
		if (progress) progress(0.0f, "Reading file");
		wi::Archive archive(fileName, true);
		if (!archive.IsOpen())
			return;

		// Serialize it from file (drives progress through the [0.05, 0.75] range):
		scene.Serialize(archive, progress);
		...
		if (progress) progress(0.90f, "Applying transform");
		{
