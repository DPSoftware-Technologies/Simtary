...
namespace wi::scene
{
	// Progress callback for LoadModel/LoadModel2 (and Scene::Serialize read), invoked repeatedly
	// across the WHOLE load so a progress bar can advance through every phase:
	//	fraction	: overall progress in [0,1] (weighted across file read, resources, components,
	//				  async asset processing, transform apply and merge)
	//	status		: human-readable label of the current phase / component (e.g. "Reading file",
	//				  "Loading resources", "Loading materials", "Processing assets", "Applying transform")
	//	The callback fires on the loading thread; keep it lightweight and thread-safe if loading asynchronously.
	using LoadModelProgressCallback = std::function<void(float fraction, const std::string& status)>;
	...
	struct Scene
	{
		...
		wi::ecs::ComponentManager<RigidBodyPhysicsComponent>& rigidbodies = componentLibrary.Register<RigidBodyPhysicsComponent>("wi::scene::Scene::rigidbodies", 7); // version = 7
		...
		// Native Component runtime state (not serialized; rebuilt from MetadataComponent each Update):
		NativeComponentManager nativeComponents;
		...
		//	progress	: optional load-progress callback (read mode only); see LoadModelProgressCallback
		void Serialize(wi::Archive& archive, LoadModelProgressCallback progress = nullptr);
		...
		void RunNativeComponentUpdateSystem(wi::jobsystem::context& ctx);
	...
	//	progress		:	optional load progress callback (optional)
	//
	//	returns INVALID_ENTITY if attached argument was false, else it returns the base entity handle
	wi::ecs::Entity LoadModel(const std::string& fileName, const XMMATRIX& transformMatrix = XMMatrixIdentity(), bool attached = false, LoadModelProgressCallback progress = nullptr);
	...
	//
	//	returns INVALID_ENTITY if attached argument was false, else it returns the base entity handle
	wi::ecs::Entity LoadModel(Scene& scene, const std::string& fileName, const XMMATRIX& transformMatrix = XMMatrixIdentity(), bool attached = false, LoadModelProgressCallback progress = nullptr);
	...
	//	progress		:	optional load progress callback (optional)
	void LoadModel2(const std::string& fileName, const XMMATRIX& transformMatrix = XMMatrixIdentity(), wi::ecs::Entity rootEntity = wi::ecs::INVALID_ENTITY, LoadModelProgressCallback progress = nullptr);
	...
	//	rootEntity		:	specify entity to attach whole scene to (optional)
	//	progress		:	optional load progress callback (optional)
	void LoadModel2(Scene& scene, const std::string& fileName, const XMMATRIX& transformMatrix = XMMatrixIdentity(), wi::ecs::Entity rootEntity = wi::ecs::INVALID_ENTITY, LoadModelProgressCallback progress = nullptr);
...
// NativeComponent template definitions (need the complete Scene + engine component types):
#include "stNativeComponent_inl.h"
