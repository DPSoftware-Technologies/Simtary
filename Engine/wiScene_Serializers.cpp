...
namespace wi::scene
{
	...
	void Scene::Serialize(wi::Archive& archive, LoadModelProgressCallback progress)
	{
		wi::Timer timer;
		const bool report_progress = progress && archive.IsReadMode();
		...
		{
			if (report_progress) progress(0.05f, "Loading resources");
		...
		// Map per-component deserialization (componentLibrary path) into the [0.10, 0.70] band of overall load progress:
		if (report_progress)
		{
			seri.progress = [progress](uint32_t total, uint32_t current, const std::string& name) {
				const float f = 0.10f + 0.60f * (total ? float(current) / float(total) : 0.0f);
				const size_t sep = name.rfind("::"); // "wi::scene::Scene::materials" -> "materials"
				progress(f, "Loading " + (sep == std::string::npos ? name : name.substr(sep + 2)));
			};
		}
		...
		if (report_progress) progress(0.75f, "Processing assets");
